...
The Random Forest model achieved the highest AUC (`~0.98`), perfect sensitivity (`1.00`), and relatively higher specificity (0.39), indicating strong performance in both detecting Okataina samples and avoiding false positives.

In contrast, the kNN model yielded a slightly lower sensitivity (`0.99`) and a significantly reduced specificity (`0.23`). This suggests that kNN is more prone to incorrectly labeling non-Okataina samples as Okataina, increasing the risk of false positives. Therefore, despite similar recall, Random Forest offers more reliable overall performance.
...
The red point on the ROC curve marks the selected threshold (0.70), which relatively maximizes the Youden index. This threshold balances sensitivity and specificity, improving the model’s ability to confidently classify Okataina samples while reducing false positives.
...
The density plot of predicted probabilities by class further visualizes the separation between "Yes" and "No" samples relative to the threshold.
...
With the adjusted threshold set at 0.70, the model classifies a reasonable number of unlabelled samples as “Okataina”, with 21 predictions compared to 27 under the default threshold. This adjustment enhances probabilistic confidence in model's positive classifications.
...
Future improvements could include cost-sensitive learning, synthetic balancing techniques (e.g., SMOTE), and model interpretation tools such as SHAP or LIME to enhance transparency in geological applications.
