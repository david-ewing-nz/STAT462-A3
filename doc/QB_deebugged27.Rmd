...
# Align clusters with annotated classes using majority vote
library(dplyr)
if (exists("annotated")) {
  # Merge annotated labels into clustering output
label_col <- "Class"
  annotated_subset <- annotated %>% filter(!is.na(Class))
  ...
  mapping <- annotated_subset %>%
    ...
    ungroup()
  cluster_map <- setNames(as.character(mapping$Class), mapping$cluster)
  # Apply mapping to unlabeled_use
  unlabeled_use$label <- cluster_map[as.character(res_final$clusters)]
}
...
```{r res-summary, echo=TRUE}
cat("Converged in", res_final$iter, "iterations\n")
cat("Cluster sizes:\n")
print(table(res_final$clusters))
```

```{r final-clustering-7}
k_best <- 7
res_final <- f_kmeans(
  unlabeled_scaled,
  k_best,
